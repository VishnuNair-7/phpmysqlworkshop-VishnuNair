...
<html lang="en" dir="ltr">
  ...
  <br>
  <br>
  <input class="btn btn-lg btn-dark" type="submit" name="" value="Delete">
    </form>
  </body>
</html>
<?php
    ...
    if(isset($_POST['email'])){
      ...
      $emailcount = mysqli_num_rows($checkemail);
        if(filter_var($email,FILTER_VALIDATE_EMAIL)){
            if($emailcount==1){
              $del = "DELETE FROM students WHERE email='$email'";
              $del1 = "DELETE FROM creds WHERE emaiid='$email'";
              $delquery = mysqli_query($db,$del);
              $delquery1 = mysqli_query($db,$del);
              if($delquery){
                ?><script>
              ...
              ?><script>
                alert("Email is not present in student databases")
              </script><?php
            }
